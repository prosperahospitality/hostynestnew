...
const columns = [
    ...
    {name: "DISTRICT DESCRIPTION", uid: "district_desc", sortable: true},
...
export default function District() {
    ...
    const initialFxn = async () => {
        try {
            const response = await fetch("/api/locationmaster/locationmaster_district", {
                method: "GET",
                headers: {
    ...
    const generateUniqueID = () => {
      ...
      const newID = `LD${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
            ...
            if(actionType === "add") {
                ...
                try {
                    const response = await fetch("/api/locationmaster/locationmaster_district", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, district: district,
        ...
        useEffect(() => {
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=LD)0*(\d+)/); // Extract numeric part using regular expression
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.district,item.district_desc,item.status);
                    }}
                  >
    ...
    return (
        ...
        reverseOrder={false} /><div className="w-full">
          <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add district</h1>
        ...
        setStatus('')
            ...
            <Modal isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
              <ModalContent>
                {(onClose) => (
                  <>
                    <ModalHeader className="gap-1">Add district</ModalHeader>
                    <ModalBody>
                      <div className="p-4 grid grid-cols-2 gap-2">
                        {actionType === "editmany" ? '' :
                          <><Input
                            ...
                            size="md"
                            className="max-w-xs"
                            value={actionType === "edit" ? district : district}
                            ...
                            <Textarea
                              ...
                              value={actionType === "edit" ? districtDesc : districtDesc}
...
<DataTable data={result} columns={columns}
  statusOptions={statusOptions} statusColorMap={statusColorMap} columnSort="id" columnName={"district"} actionsContent={actionsContent} operation="district" handleOpen={handleOpen} handleClick={handleClick} handleDelete={handleDelete} handleSubmit={handleSubmit} />
</div>
