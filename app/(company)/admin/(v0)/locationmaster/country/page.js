...
const columns = [
    ...
    {name: "COUNTRY DESCRIPTION", uid: "country_desc", sortable: true},
...
export default function Country() {
    ...
    const initialFxn = async () => {
        try {
            const response = await fetch("/api/locationmaster/locationmaster_country", {
                method: "GET",
                headers: {
            ...
            if(actionType === "add") {
                ...
                try {
                    const response = await fetch("/api/locationmaster/locationmaster_country", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, country: country,
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.country,item.country_desc,item.status);
                    }}
                  >
    ...
    return (
        ...
        reverseOrder={false} /><div className="w-full">
          <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add country</h1>
        ...
        setStatus('')
            ...
            <Modal isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
              <ModalContent>
                {(onClose) => (
                  <>
                    <ModalHeader className="gap-1">Add country</ModalHeader>
                    <ModalBody>
                      <div className="p-4 grid grid-cols-2 gap-2">
                        {actionType === "editmany" ? '' :
                          <><Input
                            ...
                            size="md"
                            className="max-w-xs"
                            value={actionType === "edit" ? country : country}
                            ...
                            <Textarea
                              ...
                              value={actionType === "edit" ? countryDesc : countryDesc}
          ...
          <div className="mt-10 ml-2 mr-2">
            <DataTable data={result} columns={columns}
              statusOptions={statusOptions} statusColorMap={statusColorMap} columnSort="id" columnName={"country"} actionsContent={actionsContent} operation="country" handleOpen={handleOpen} handleClick={handleClick} handleDelete={handleDelete} handleSubmit={handleSubmit} />
          </div>
        </div></>
    );
};