...
const columns = [
    ...
    {name: "CITY DESCRIPTION", uid: "city_desc", sortable: true},
    {name: "PINCODE", uid: "city_pincode", sortable: true},
...
export default function City() {
    ...
    const initialFxn = async () => {
        try {
            const response = await fetch("/api/locationmaster/locationmaster_city", {
                method: "GET",
                headers: {
    ...
    const generateUniqueID = () => {
      ...
      const newID = `LC${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
            ...
            if(actionType === "add") {
                ...
                try {
                    const response = await fetch("/api/locationmaster/locationmaster_city", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, city: city,
                          cityDesc: cityDesc,
                          city_pincode: cityPincode,
                            status: status}),
                    });
        ...
        useEffect(() => {
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=LC)0*(\d+)/); // Extract numeric part using regular expression
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.city,item.city_desc,item.status);
                    }}
                  >
    ...
    return (
        ...
        reverseOrder={false} /><div className="w-full">
          <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add city</h1>
        ...
        setStatus('')
            ...
            <Modal isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
              <ModalContent>
                {(onClose) => (
                  <>
                    <ModalHeader className="gap-1">Add city</ModalHeader>
                    <ModalBody>
                      <div className="p-4 grid grid-cols-2 gap-2">
                        {actionType === "editmany" ? '' :
                          <><Input
                            ...
                            size="md"
                            className="max-w-xs"
                            value={actionType === "edit" ? city : city}
                            ...
                            <Textarea
                              ...
                              value={actionType === "edit" ? cityDesc : cityDesc}
                            ...
                            size="md"
                            className="max-w-xs"
                            value={actionType === "edit" ? cityPincode : cityPincode}
          ...
          <div className="mt-10 ml-2 mr-2">
            <DataTable data={result} columns={columns}
              statusOptions={statusOptions} statusColorMap={statusColorMap} columnSort="id" columnName={"city"} actionsContent={actionsContent} operation="city" handleOpen={handleOpen} handleClick={handleClick} handleDelete={handleDelete} handleSubmit={handleSubmit} />
          </div>
        </div></>
    );
};