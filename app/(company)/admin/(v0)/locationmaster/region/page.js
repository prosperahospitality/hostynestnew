...
const columns = [
    ...
    {name: "REGION DESCRIPTION", uid: "region_desc", sortable: true},
...
export default function Region() {
    ...
    const initialFxn = async () => {
        try {
            const response = await fetch("/api/locationmaster/locationmaster_region", {
                method: "GET",
                headers: {
    ...
    const generateUniqueID = () => {
      ...
      const newID = `LR${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
            ...
            if(actionType === "add") {
                ...
                try {
                    const response = await fetch("/api/locationmaster/locationmaster_region", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, region: region,
        ...
        useEffect(() => {
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=LR)0*(\d+)/); // Extract numeric part using regular expression
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.region,item.region_desc,item.status);
                    }}
                  >
    ...
    return (
        ...
        reverseOrder={false} /><div className="w-full">
          <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add Region</h1>
                      ...
                      <div className="p-4 grid grid-cols-2 gap-2">
                        {actionType === "editmany" ? '' :
                          <><Input
                            ...
                            size="md"
                            className="max-w-xs"
                            value={actionType === "edit" ? region : region}
                            ...
                            <Textarea
                              ...
                              value={actionType === "edit" ? regionDesc : regionDesc}
