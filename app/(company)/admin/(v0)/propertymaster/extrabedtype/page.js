...
const columns = [
    ...
    {name: "EXTRA BED DESCRIPTION", uid: "propertyextbedtype_desc", sortable: true},
  ...
  const extrabedtypesdata = [
    {
      id: "EBTID00001",
      extrabedtype: "Mattress",
      extrabeddescription: "Mattress",
      status: "active",
    },
    {
        id: "EBTID00002",
        extrabedtype: "Twin Bed",
        extrabeddescription: "Twin Bed",
        status: "active",
    },
    {
        id: "EBTID00003",
        extrabedtype: "Double Bed",
        extrabeddescription: "Double Bed",
        status: "inactive",
    },
  ];


export default function ExtraBedType() {
    ...
    const initialFxn = async () => {
        try {
            const response = await fetch("/api/property/property_extbedtype", {
                method: "GET",
                headers: {
    ...
    function getCurrentDateTime() {
      ...
      const newID = `EBTID${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
        ...
        const handleSubmit = async () => {
            ...
            if(actionType === "add") {
                ...
                const data = {
                    id: generateUniqueID(),
                    property_extbedtype: propertyExtbedtype,
                    propertyextbedtype_desc: propertyDesc,
                    status: status,
                ...
                try {
                    const response = await fetch("/api/property/property_extbedtype", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, property_extbedtype: propertyExtbedtype,
                            propertyextbedtype_desc: propertyDesc,
                            status: status}),
                    });
                    const result = await response.json();
            ...
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=EBTID)0*(\d+)/); // Extract numeric part using regular expression
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.property_extbedtype,item.propertyextbedtype_desc,item.status);
                    }}
                  >
    ...
    return (
        ...
        <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add Extra Bed Type</h1>
        <div className="mr-6 text-end">
                {/* <Button onPress={() => handleOpen("add")} color="primary" variant="shadow" onClick={(e) => {
                    ...
                    setPropertyDesc('')
                    setStatus('')
                }}><PlusIcon className="size-4" />Add Extra Bed Type</Button> */}
                <Modal isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
                    <ModalContent>
                        {(onClose) => (
                            <>
                                <ModalHeader className="gap-1">Add Extra Bed</ModalHeader>
                                <ModalBody>
                                    <div className="p-4 grid grid-cols-2 gap-2">
                                        ...
                                        <><Input
                                            ...
                                            size="md"
                                            className="max-w-xs"
                                            value={actionType === "edit" ? propertyExtbedtype : propertyExtbedtype}
                                            onChange={(e) => setPropertyExtbedtype(e.target.value)}
                                        />
                                        <Textarea
                                            isRequired
                                            type="text"
                                            label="Extra Bed Description"
                                            labelPlacement="outside"
                                            variant="bordered"
                                            placeholder="Extra Bed description"
                                            disableAnimation
                                            disableAutosize
                                            classNames={{
                                                base: "max-w-xs",
  ...
  statusOptions = {statusOptions} statusColorMap = {statusColorMap} columnSort = "id" columnName = {"property_extbedtype"} actionsContent = {actionsContent} operation = "propExtBedType" handleOpen = {handleOpen} handleClick = {handleClick} handleDelete = {handleDelete} handleSubmit = {handleSubmit}/>
            </div>
        </div></>
    );
};