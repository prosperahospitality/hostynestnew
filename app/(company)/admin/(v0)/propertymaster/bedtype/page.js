...
const columns = [
    ...
    {name: "BED DESCRIPTION", uid: "propertybedtype_desc", sortable: true},
  ...
  const bedtypesdata = [
    {
      id: "BTID00001",
      bedtype: "Single Bed",
      beddescription: "Single Bed",
      status: "active",
    },
    {
        id: "BTID00002",
        bedtype: "King Size Bed",
        beddescription: "King Size Bed",
        status: "active",
    },
    {
        id: "BTID00003",
        bedtype: "Double Bed",
        beddescription: "Double Bed",
        status: "inactive",
    },
  ];
...
export default function BedType() {
    ...
    const initialFxn = async () => {
        try {
            const response = await fetch("/api/property/property_bedtype", {
                method: "GET",
                headers: {
    ...
    function getCurrentDateTime() {
      ...
      const newID = `BTID${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
        ...
        const handleSubmit = async () => {
            ...
            if(actionType === "add") {
                ...
                const data = {
                    id: generateUniqueID(),
                    property_bedtype: propertyBedtype,
                    propertybedtype_desc: propertyDesc,
                    status: status,
                ...
                try {
                    const response = await fetch("/api/property/property_bedtype", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, property_bedtype: propertyBedtype,
                            propertybedtype_desc: propertyDesc,
                            status: status}),
                    });
                    const result = await response.json();
            ...
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=BTID)0*(\d+)/); // Extract numeric part using regular expression
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.property_bedtype,item.propertybedtype_desc,item.status);
                    }}
                  >
    ...
    return (
        <><Toaster
            ...
            reverseOrder={false} /><div className="w-full">
                <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add Bed Type</h1>
        ...
        setPropertyDesc('')
        setStatus('')
    }}><PlusIcon className="size-6" />Add Bed Type</Button> */}
                    <Modal isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
                        <ModalContent>
                            {(onClose) => (
                                <>
                                    <ModalHeader className="gap-1">Add Bed</ModalHeader>
                                    <ModalBody>
                                        <div className="p-4 grid grid-cols-2 gap-2">
                                            {actionType === "editmany" ? '' :
                                                <><Input
                                                    ...
                                                    size="md"
                                                    className="max-w-xs"
                                                    value={actionType === "edit" ? propertyBedtype : propertyBedtype}
                                                    onChange={(e) => setPropertyBedtype(e.target.value)} /><Textarea
                                                        isRequired
                                                        type="text"
                ...
                <div className="mt-10 ml-2 mr-2">
                    ...
                    <DataTable data={result} columns={columns}
                        statusOptions={statusOptions} statusColorMap={statusColorMap} columnSort="id" columnName={"property_bedtype"} actionsContent={actionsContent} operation="propBedType" handleOpen={handleOpen} handleClick={handleClick} handleDelete = {handleDelete} handleSubmit = {handleSubmit}/>
                </div>
            </div></>
    );
};