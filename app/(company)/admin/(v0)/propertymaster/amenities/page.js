...
const columns = [
    ...
    {name: "AMENITIES", uid: "amenities", sortable: true},
    {name: "AMENITIES DESCRIPTION", uid: "amenities_desc", sortable: true},
  ...
  const amenitesdata = [
    ...
    },
    {
        ...
        value: "iron",
        amenitestype: "Iron",
        amenitesdescription: "Iron",
        status: "active",
    },
    {
        id: "AMID00003",
        value: "hottub",
        amenitestype: "Hot tub",
        amenitesdescription: "Hot tub",
        status: "inactive",
    },
  ];
...
checksRef.current = useSelector((state) => state.checks.selectedChecks);
    ...
    function getCurrentDateTime() {
      ...
      const newID = `AMID${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
        ...
        const handleSubmit = async () => {
            ...
            if(actionType === "add") {
                ...
                const data = {
                    id: generateUniqueID(),
                    amenities: amenities,
                    amenities_desc: amenitiesDesc,
                    value: amenities.toLowerCase().replace(/\s/g, ''),
                    status: status,
                ...
                try {
                    const response = await fetch("/api/property/amenities", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, amenities: amenities,
                            amenities_desc: amenitiesDesc, value : amenities.toLowerCase().replace(/\s/g, ''),
                            status: status}),
                    });
                    const result = await response.json();
            ...
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=AMID)0*(\d+)/); // Extract numeric part using regular expression
        ...
        const rowEdit = async (key,type,desc,val) => {
            setCurrRowId(key)
            setAmenities(type)
            setAmenitiesDesc(desc)
            // setValue(val)
        }
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.amenities,item.amenities_desc, item.value);
                    }}
                  >
 ...
 <><Toaster
        ...
        <div className="w-full">
            <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add Amenities</h1>
            <div className="mr-6 text-end">
                {/* <Button onPress={() => handleOpen("add")} color="primary" variant="shadow" onClick={(e) => {setAmenities(''),
                    setAmenitiesDesc('')
                }}>
            <PlusIcon className="size-6" />Add Amenities</Button> */}
                <Modal isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
                    <ModalContent>
                        {(onClose) => (
                            <>
                                <ModalHeader className="gap-1">Add Amenities</ModalHeader>
                                <ModalBody>
                                    ...
                                    <>
                                        <Input
                                            ...
                                            size="md"
                                            className="max-w-xs"
                                            value={actionType === "edit" ? amenities: amenities}
                                        ...
                                        {/* <Input
                                            ...
                                            size="md"
                                            className="max-w-xs"
                                            value={actionType === "edit" ? value : value}
                                        ...
                                        <Textarea
                                            ...
                                            value={actionType === "edit" ? amenitiesDesc : amenitiesDesc}
                                            onChange={(e) => setAmenitiesDesc(e.target.value)}
                                            /></>}
                                        <Autocomplete
                                        isRequired
                                        labelPlacement="outside"
                                        placeholder="Select...."
                                        label="Amenities Type Status"
                                        variant="bordered"
                                        size="md"
                                        className="max-w-xs" 
  ...
  statusOptions = {statusOptions} statusColorMap = {statusColorMap} columnSort = "id" columnName = {"amenities"} actionsContent = {actionsContent} operation = "propAmenities" handleOpen = {handleOpen} handleClick = {handleClick} handleDelete = {handleDelete} handleSubmit = {handleSubmit}/>
            </div>
        </div></>
    );
};