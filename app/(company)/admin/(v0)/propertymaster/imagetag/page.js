...
const columns = [
    ...
    {name: "PROPERTY AREA", uid: "tag_name", sortable: true},
    {name: "PROPERTY AREA DESCRIPTION", uid: "tag_desc", sortable: true},
...
export default function ImageTagPage() {
    ...
    const generateUniqueID = () => {
      ...
      const newID = `PAIT${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
            ...
            if(actionType === "add") {
                ...
                try {
                    const response = await fetch("/api/property/image_tag", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, tag_name: tagName,
                            tag_desc: tagDesc,
                            status: status}),
                    });
                    const result = await response.json();
        ...
        useEffect(() => {
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=PAIT)0*(\d+)/); // Extract numeric part using regular expression
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.tag_name,item.tag_desc,item.status);
                    }}
                  >
    ...
    return (
        ...
        reverseOrder={false} /><div className="w-full">
          <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add Image Tag</h1>
        ...
        setStatus('')
                      ...
                      <div className="p-4 grid grid-cols-2 gap-2">
                        {actionType === "editmany" ? '' :
                          <><Input
                            ...
                            size="md"
                            className="max-w-xs"
                            value={actionType === "edit" ? tagName : tagName}
                            ...
                            <Textarea
                              ...
                              value={actionType === "edit" ? tagDesc : tagDesc}
          ...
          <div className="mt-10 ml-2 mr-2">
            <DataTable data={result} columns={columns}
              statusOptions={statusOptions} statusColorMap={statusColorMap} columnSort="id" columnName={"tag_name"} actionsContent={actionsContent} operation="imageTag" handleOpen={handleOpen} handleClick={handleClick} handleDelete={handleDelete} handleSubmit={handleSubmit} />
          </div>
        </div></>
    );
};