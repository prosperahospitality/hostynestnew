...
const columns = [
    ...
    {name: "FLOOR", uid: "property_floor", sortable: true},
    {name: "FLOOR DESCRIPTION", uid: "propertyfloor_desc", sortable: true},
...
export default function Floor() {
    ...
    function getCurrentDateTime() {
      ...
      const newID = `FID${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
        ...
        const handleSubmit = async () => {
            ...
            if(actionType === "add") {
                ...
                const data = {
                    ...
                    propertyfloor_desc: propertyDesc,
                    status: status,
                ...
                try {
                    const response = await fetch("/api/property/property_floor", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, property_floor: propertyFloor,
                            propertyfloor_desc: propertyDesc,
                            status: status}),
                    });
                    const result = await response.json();
            ...
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=FID)0*(\d+)/); // Extract numeric part using regular expression
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.property_floor,item.propertyfloor_desc,item.status);
                    }}
                  >
    ...
    return (
        ...
        reverseOrder={false} /><div className="w-full">
          <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add Floor</h1>
        ...
        setStatus('')
            ...
            <Modal isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
              <ModalContent>
                {(onClose) => (
                  <>
                    <ModalHeader className="gap-1">Add Floor</ModalHeader>
                    <ModalBody>
                      ...
                      {actionType === "editmany" ? '' :
                        <><Input
                            ...
                            size="md"
                            className="max-w-xs"
                            value={actionType === "edit" ? propertyFloor : propertyFloor}
                            onChange={(e) => setPropertyFloor(e.target.value)} /><Textarea
                              isRequired
                              type="text"
          ...
          <div className="mt-10 ml-2 mr-2">
            <DataTable data={result} columns={columns}
              statusOptions={statusOptions} statusColorMap={statusColorMap} columnSort="id" columnName={"property_floor"} actionsContent={actionsContent} operation="propType" handleOpen={handleOpen} handleClick={handleClick} handleDelete = {handleDelete} handleSubmit = {handleSubmit}/>
          </div>
        </div></>
    );
};