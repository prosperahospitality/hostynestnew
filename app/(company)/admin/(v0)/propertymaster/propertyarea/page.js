...
const columns = [
    ...
    {name: "PROPERTY AREA", uid: "property_area", sortable: true},
    {name: "PROPERTY AREA DESCRIPTION", uid: "propertyarea_desc", sortable: true},
...
export default function PropertyAreaPage() {
    ...
    const initialFxn = async () => {
        try {
            const response = await fetch("/api/property/property_area", {
                method: "GET",
                headers: {
    ...
    const generateUniqueID = () => {
      ...
      const newID = `PAID${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
        ...
        const handleSubmit = async () => {
            ...
            if(actionType === "add") {
                ...
                const data = {
                    ...
                    propertyarea_desc: propertyDesc,
                    status: status,
                ...
                try {
                    const response = await fetch("/api/property/property_area", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, property_area: propertyArea,
                            propertyarea_desc: propertyDesc,
                            status: status}),
                    });
                    const result = await response.json();
        ...
        useEffect(() => {
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=PAID)0*(\d+)/); // Extract numeric part using regular expression
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.property_area,item.propertyarea_desc,item.status);
                    }}
                  >
    ...
    return (
        ...
        reverseOrder={false} /><div className="w-full">
          <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add Property Area</h1>
        ...
        setStatus('')
                      ...
                      <div className="p-4 grid grid-cols-2 gap-2">
                        {actionType === "editmany" ? '' :
                          <><Input
                            ...
                            size="md"
                            className="max-w-xs"
                            value={actionType === "edit" ? propertyArea : propertyArea}
                            onChange={(e) => setPropertyArea(e.target.value)} /><Textarea
                              isRequired
                              type="text"
                              label="Property Area Description"
                              labelPlacement="outside"
                              variant="bordered"
                              placeholder="Property Area description"
                              disableAnimation
                              disableAutosize
                              classNames={{
                                base: "max-w-xs",
          ...
          <div className="mt-10 ml-2 mr-2">
            <DataTable data={result} columns={columns}
              statusOptions={statusOptions} statusColorMap={statusColorMap} columnSort="id" columnName={"property_area"} actionsContent={actionsContent} operation="propArea" handleOpen={handleOpen} handleClick={handleClick} handleDelete={handleDelete} handleSubmit={handleSubmit} />
          </div>
        </div></>
    );
};