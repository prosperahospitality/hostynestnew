...
const columns = [
    ...
    {name: "PROPERTY ROOM NAME DESCRIPTION", uid: "property_roomname_desc", sortable: true},
...
export default function Roomname() {
    ...
    const [ roomNameDesc, setRoomNameDesc ] = useState();
    ...
    const initialFxn = async () => {
        try {
            const response = await fetch("/api/property/property_roomname", {
                method: "GET",
                headers: {
    ...
    const generateUniqueID = () => {
      ...
      const newID = `PRNID${String(lastID + 1).padStart(5, '0')}`;
      setLastID(lastID + 1);
      return newID;
    };
        ...
        const handleSubmit = async () => {
            ...
            if(actionType === "add") {
                ...
                const data = {
                    id: generateUniqueID(),
                    property_roomname: roomName,
                    property_roomname_desc: roomNameDesc,
                    status: status,
                ...
                try {
                    const response = await fetch("/api/property/property_roomname", {
                        ...
                        body: JSON.stringify({id: currRowId, action: actionType, property_roomname: roomName,
                          property_roomname_desc: roomNameDesc,
                            status: status}),
                    });
                    const result = await response.json();
        ...
        useEffect(() => {
            if (result && result.length > 0) {
                ...
                const numericPart = lastElementId.match(/(?<=PRNID)0*(\d+)/); // Extract numeric part using regular expression
...
console.log(key,type,desc)
              ...
              <Tooltip color="default" content="Edit Bed Type">
                <span className="text-lg text-default-400 cursor-pointer active:opacity-50">
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => {
                      rowEdit(item.id,item.property_roomname,item.property_roomname_desc);
                    }}
                  >
    ...
    return (
        ...
        <div className="w-full">
            <h1 className="flex text-3xl ml-6 mt-6"><PlusIcon className="size-8 mt-1" />Add Room Names</h1>
            <div className="mr-6 text-end">
                <Modal isOpen={isOpen} size="4xl" onOpenChange={onOpenChange}>
                    <ModalContent>
                        {(onClose) => (
                            <>
                                <ModalHeader className="gap-1">Add Room Names</ModalHeader>
                                <ModalBody>
                                    <div className="p-4 grid grid-cols-2 gap-2">
                                        ...
                                        <Input
                                            ...
                                            size="md"
                                            className="max-w-xs"
                                            value={actionType === "edit" ? roomName : roomName}
                                            onChange={(e) => setRoomName(e.target.value)}
                                        />
                                        <Textarea
                                            isRequired
                                            type="text"
                                            label="Room Name Description"
                                            labelPlacement="outside"
                                            variant="bordered"
                                            ...
                                            value={actionType === "edit" ? roomNameDesc : roomNameDesc}
                                            onChange={(e) => setRoomNameDesc(e.target.value)}
                                            /></>}
                                        <Autocomplete
                                        isRequired
                                        labelPlacement="outside"
  ...
  statusOptions = {statusOptions} statusColorMap = {statusColorMap} columnSort = "id" columnName = {"property_roomname"} actionsContent = {actionsContent} operation = "roomName" handleOpen = {handleOpen} handleClick = {handleClick} handleDelete = {handleDelete} handleSubmit = {handleSubmit}/>            
            </div>
        </div></>
    );
};